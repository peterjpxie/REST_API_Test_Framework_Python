...
class TestAPI:
    ...
    def test_by_input_output_text(self,testcase_folder):
        """ test by input and output(expect) text files

        Write only this test function and use parametrize method to test different cases by:
        - read input request text files
        - compare output with expected output text files where json format is converted to ini format for easy comparison
        """        
        # post
        input_root = '../input'
        output_root = '../output'
    ...
    def request(self, method, url, headers={}, data=None, amend_headers=True, verify=False, **kwargs):
        """
...
if __name__ == '__main__':
    ...
    # get
    method, url, headers, body = parse_test_input('../inputs/test_case_02/request_01.txt')
